		...
		Overlay = function() {
			...
			if ($jq(".fs-grid").length < 1) {
				...
				if ($jq("#fs-grid-styles").length < 1) {
					$jq("body").append('<link id="fs-grid-styles" rel="stylesheet" href="//formstone.it/bookmarklet/grid.css" type="text/css" media="all">');
				}

				if ($jq("#fs-grid-overlay").length < 1) {
					var html = '';

					html += '<aside id="fs-grid-menu" class="' + config.position + '">';
