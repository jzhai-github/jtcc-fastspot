...
(function ($, Site) {
	var $Options, $Toggle, $Tools, Description, Title, URL;
	...
	function init() {
			...
			if (typeof navigator.share === 'undefined') {
				setTimeout(function () {
					if ($Tools.hasClass('fs-swap-active')) {
						...
						$('.js-share-tool-item:first-child').transition(
							{ always: false, property: 'opacity' },
							function () {
		...
		function onToggleKeydown(e) {
			...
			if (typeof navigator.share === 'undefined') {
				...
				if (key === 38) {
					if ($Tools.hasClass('fs-swap-active')) {
						$('.js-share-tool-item:last-child').transition(
							{ always: false, property: 'opacity' },
							function () {
		...
		function onOptionsKeydown(e) {
			var key = e.keyCode;
			var focusedIndex = $(':focus')
				.closest('.js-share-tool-item')
				.index();
