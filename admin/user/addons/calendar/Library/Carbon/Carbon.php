...
namespace Solspace\Addons\Calendar\Library\Carbon;
 ...
 * @property integer       $dayOfWeek   0 (for Sunday) through 6 (for Saturday)
 * @property integer       $dayOfYear   0 through 365
 * @property integer       $weekOfYear  ISO-8601 week number of year, weeks starting on Monday
 * @property integer       $daysInMonth number of days in the given month
 * @property integer       $age         does a diffInYears() with default parameters
 * @property integer       $quarter     the quarter of this instance, 1 - 4
 * @property integer       $offset      the timezone offset in seconds from UTC
 * @property integer       $offsetHours the timezone offset in hours from UTC
 * @property boolean       $dst         daylight savings time indicator, true if DST, false otherwise
 * @property boolean       $local       checks if the timezone is local, true if local, false otherwise
 * @property boolean       $utc         checks if the timezone is UTC, true if UTC, false otherwise
...
class Carbon extends DateTime
{
    ...
    /**
     * Compares this to another date
     * Returns 1 if this is greater than $dt,
     * -1 if it's less
     * 0 if both are equal
     *
     * @param Carbon $dt
    ...
    /**
     * Determines if the current Carbon date is in the month and year of the given $date
     *
     * @param Carbon $date
     *
     * @return bool
     */
    public function inInGivenMonth(Carbon $date)
    {
        return $this->format('Ym') === $date->format('Ym');
    }
     ...
     * @return integer
     */
    public function diffInDays(Carbon $dt = null, $abs = true, $ignoreTime = false)
    {
        $dt = ($dt === null) ? static::now($this->tz) : $dt->copy();
        $self = $this->copy();
        ...
        return intval($self->diff($dt, $abs)->format('%r%a'));
    }
