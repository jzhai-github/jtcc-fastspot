...
namespace Solspace\Addons\Calendar\Library\When;
...
class When extends \DateTime
{
    ...
    public function byday($bywdaylist, $delimiter = ",")
    {
        ...
        throw new \InvalidArgumentException(
            "bydays: Accepts (optional) positive and negative values between 1 and 53 followed by a valid week day"
        );
    }
    ...
    public function rrule($rrule)
    {
        ...
        foreach ($parts as $part) {
            ...
            switch ($rule) {
                case "UNTIL":
                    $this->until(new Carbon($param, DateTimeHelper::TIMEZONE_UTC));
                    break;
                case "FREQ":
                case "COUNT":
                case "INTERVAL":
            ...
            if ($this->freq === "yearly") {
                ...
                if ($this->freq === "monthly") {
                    ...
                    if ($this->freq === "weekly") {
                        ...
                        $dateLooper = clone $this->startDate;
                        $dateLooper->setDate(
                            $startWeekDay->format("Y"),
                            $startWeekDay->format("n"),
                            $startWeekDay->format('j')
                        );
    ...
    protected function prepareDateElements()
    {
        ...
        if (!isset($this->freq) && Valid::byFreqValid(
                $this->freq,
                $this->byweeknos,
                $this->byyeardays,
                $this->bymonthdays
...
class FrequencyRequired extends \Exception
{
    public function __construct(
        $message = "You are required to set a frequency.",
        $code = 0,
...
{
    public function __construct(
        $message = "The start date must be the first occurrence.",
        $code = 0,
        Exception $previous = null
    ) {
        parent::__construct($message, $code, $previous);
