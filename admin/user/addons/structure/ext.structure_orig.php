...
use EEHarbor\Structure\FluxCapacitor\Base\Ext;
...
class Structure_ext extends Ext
{
    ...
    public function __construct($settings = '')
    {
        ...
        if (!$this->flux->module_installed('structure')) {
            return false;
        }
...
// rob1 patch
    ...
    public function cp_member_login()
    {
        ...
        if (isset($settings['redirect_on_login']) && $settings['redirect_on_login'] == 'y') {
            ee()->functions->redirect($this->flux->moduleURL());
        }
    }
    ...
    private function _create_global_vars($isLivePreviewRequest = false)
    {
        ...
        ee()->config->_global_vars['structure:page:template_id']        = $this->entry_id !== false && isset($this->site_pages['templates'][$this->entry_id]) ? $this->site_pages['templates'][$this->entry_id] : false; // {page:template_id}
