...
class Hop_404_reporter_helper
{
	...
	{
		return ['referrer_not_specified', 'referrer_not_tracked'];
	}
	
	/**
	 * Get the possible values of an email notification frequency parameter
	 */
	...
	public static function save_settings($settings = [])
	{
		...
		{
			$query = ee()->db->get_where('hop_404_reporter_settings', ['setting_name' => $setting_name], 1, 0);
			if ($query->num_rows() == 0) {
			  // A record does not exist, insert one.
			  $query = ee()->db->insert('hop_404_reporter_settings', ['setting_name' => $setting_name, 'value' => $setting_value]);
			} else {
			  // A record does exist, update it.
			  $query = ee()->db->update('hop_404_reporter_settings', ['value' => $setting_value], ['setting_name' => $setting_name]);
			}
		}
	...
	public static function save_404_url($url_404, $referrer_url, $date)
	{
		...
		$query = ee()->db->get_where('hop_404_reporter_urls', ['url' => $url_404, 'referrer_url' => $referrer_url], 1, 0);
		if ($query->num_rows() == 0)  {
			$query = ee()->db->insert('hop_404_reporter_urls', ['url' => $url_404, 'count' => 1, 'referrer_url' => $referrer_url, 'last_occurred' => $date->format('Y-m-d H:i:s')]);
			return ['is_new' => true, 'url_id' => ee()->db->insert_id()];
		} else {
			$result_array = $query->result_array();
			$count = intval($result_array[0]['count']);
			$query = ee()->db->update('hop_404_reporter_urls', ['count' => $count + 1, 'last_occurred' => $date->format('Y-m-d H:i:s')], ['url_id' => $result_array[0]['url_id']]);
			return ['is_new' => false, 'url_id' => $result_array[0]['url_id']];
		}
	}
	...
	public static function send_email_notifications($url = '', $referrer_url, $datetime, $url_id)
	{
		...
		$notif_query = ee()->db->query('SELECT * FROM `' . ee()->db->dbprefix . 'hop_404_reporter_emails` WHERE (? REGEXP `url_to_match` OR `url_to_match` = "")' . (in_array($referrer_url, ['referrer_not_specified', 'referrer_not_tracked']) ? ' AND referrer = "n"' : ''), [$url]);
		...
		$variables = [
			...
			'404_date'		=> $datetime->format('Y-m-d'),
			'404_time'		=> $datetime->format('H:i:s'),
			'ip_address'	=> ee()->input->ip_address()
		];

		$email_txt = ee()->TMPL->parse_variables_row($email_template, $variables);
		...
		$query = ee()->db->get_where('hop_404_reporter_urls', ['url_id' => $url_id], 1, 0);
		...
		foreach ($notif_query->result_array() as $row) {
			...
			if ($row['frequency'] == 'once' && $send_email) {
