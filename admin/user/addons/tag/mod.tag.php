...
use EEHarbor\Tag\FluxCapacitor\Base\Mod;
...
class Tag extends Mod
{
    ...
    public function tag_name()
    {
        ...
        if (ee()->TMPL->fetch_param('tag_separator') !== false &&
    ...
    public function tags($preview = false, $preview_tags = '')
    {
        ...
        if (ee()->TMPL->fetch_param('type') !== false &&
        ...
        if (ee()->TMPL->fetch_param('websafe_separator') !== false &&
        ...
        $sql = "/* Add-on Tag - tags() */ SELECT SQL_CALC_FOUND_ROWS t.tag_name,
                            t.tag_id,
                            t.tag_name AS tag,
                            t.channel_entries,
                            t.total_entries,
        ...
        if (ee()->TMPL->fetch_param('exclude') !== false &&
        ...
        } elseif (ee()->TMPL->fetch_param('tag_group_name')) {
            $group_ids = array();
        ...
        $order_by_options = array('clicks', 'edit_date', 'entry_date', 'total_entries', 'channel_entries');
        ...
        if (preg_match("/P(\d+)/s", ee()->uri->uri_string, $match) &&
            ...
            if ($this->p_page == 0 && is_numeric($match[1])) {
                $this->p_page = $match[1];
            }
        } else {
            $this->p_page = 0;
        ...
        //  ----------------------------------------
        $total_query = ee()->db->query("/* Add-on Tag - tags() */ SELECT FOUND_ROWS() as total_rows");
    ...
    public function entries()
    {
        ...
        if (in_array(ee()->TMPL->fetch_param('tag_rank'), array('clicks', 'total_entries', 'channel_entries'))) {
        ...
        if ($this->check_yes(ee()->TMPL->fetch_param('inclusive')) === false) {
            ...
            if (preg_match("#(^|\/)C(\d+)#", $qstring, $match) && $dynamic == 'on') {
                $cat_id = $match['2'];
            }
        ...
        if (ee()->TMPL->fetch_param('show_pages') !== false &&
             in_array(ee()->TMPL->fetch_param('show_pages'), array('only', 'no')) &&
             ($pages = ee()->config->item('site_pages')) !== false) {
            //is this version 2?
            if (! array_key_exists('templates', $pages) &&
    ...
    protected function _entries($params = array())
    {
        ...
        if (isset($params['dynamic']) && $params['dynamic'] == "off") {
    ...
    public function related_entries()
    {
        ...
        if (ee()->TMPL->fetch_param('orderby') == 'relevance' &&
            ! stristr(ee()->TMPL->tagdata, LD . 'paginate' . RD) &&
        ...
        if (ee()->TMPL->fetch_param('exclude') !== false && ee()->TMPL->fetch_param('exclude') != '') {
            $ids = $this->exclude(ee()->TMPL->fetch_param('exclude'));
        ...
        if (ctype_digit(ee()->TMPL->fetch_param('rank_limit')) === true) {
            ...
            if ($rank_method == '' || (stristr($rank_method, 'total_entries') && stristr($rank_method, 'clicks'))) {
    ...
    public function cloud()
    {
        ...
        if (ee()->TMPL->fetch_param('tag_name') !== false && ee()->TMPL->fetch_param('tag_name') != '') {
        ...
        if (ee()->TMPL->fetch_param('tag_id') !== false && ee()->TMPL->fetch_param('tag_id') != '') {
    ...
    public function parse()
    {
        ...
        if (ee()->input->post('tag_separator_override') &&
            array_key_exists(
        ...
        foreach ($query->result_array() as $row) {
            ...
            if ($this->remote === false) {
                ...
                if (isset($existing_entries[$row['tag_id']]) &&
                    ...
                    //  Otherwise, if the entry does not have a reference to the tag, make it so.
                    ee()->db->insert('exp_tag_entries', $data);
                }
            } elseif (isset($existing_entries[$row['tag_id']]) === false && in_array($row['tag_name'], $this->bad()) === false) {
        ...
        if ($this->from_ft == true && $this->field_id !== 'default') {
                ...
                if (ee()->db->field_exists('field_id_' . $this->field_id, 'exp_channel_data')) {
                    ee()->db->query(
                        ee()->db->update_string(
                            'exp_channel_data',
                            array(
                                'field_id_' . $this->field_id   => $tag_names_string
                            ),
                            array(
                                'entry_id'                      => $this->entry_id
                            )
    ...
    protected function entry_id($type = 'channel')
    {
        ...
        } elseif (ee()->uri->query_string != '' || (isset(ee()->uri->page_query_string) === true && ee()->uri->page_query_string != '')) {
            $qstring = (ee()->uri->page_query_string != '') ? ee()->uri->page_query_string : ee()->uri->query_string;
    ...
    public function field_type_widget($data)
    {
        ...
        if (array_key_exists($this->model('Data')->preference('separator'), $this->model('Data')->delimiters)) {
        ...
        $this->cached_vars['entry_id'] = $entry_id = (
            is_numeric($data['entry_id']) && $data['entry_id'] > 0
        ) ? $data['entry_id'] : 0;

        $this->cached_vars['channel_id'] = $channel_id = (
            is_numeric($data['channel_id']) && $data['channel_id'] > 0
        ) ? $data['channel_id'] : 0;
        ...
        if (empty($tags) && !empty($current_tag_names) && !$data['input_only']) {
    ...
    protected function get_tag_group_id()
    {
        ...
        } elseif (isset(ee()->TMPL) &&
                is_object(ee()->TMPL) &&
                ee()->TMPL->fetch_param('tag_group_name')) {
            $tag_group_id = $this->model('Data')->get_tag_group_id_by_name(
                ee()->TMPL->fetch_param('tag_group_name')
            );
        } elseif (ee()->input->get_post('tag_group_id')) {
            $tag_group_id = ee()->input->get_post('tag_group_id');
        } elseif (ee()->input->get_post('tag_group_name')) {
            $tag_group_id = $this->model('Data')->get_tag_group_id_by_name(
        ...
        //is it legit, dawg?
        if ($tag_group_id !== false &&
            (is_numeric($tag_group_id) && $tag_group_id > 1) or
