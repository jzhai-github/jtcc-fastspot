...
class Tag_mcp extends Mcp
{
    ...
    public function index($message = '')
    {
        ...
        $this->cached_vars['box_class'] = (!empty($this->cached_vars['box_class']) ? $this->cached_vars['box_class'] : '');

        if (substr(APP_VER, 0, 1) == 3) {
            $this->cached_vars['box_class'] .= ' box';
        }
    ...
    public function edit_tag_group()
    {
        ...
        if (ee()->input->get_post('tag_group_id') and
            ...
            if (substr(APP_VER, 0, 1) === 3) {
                ...
                $group_name = strtolower(ee('Format')->make('Text', ee()->input->get_post('tag_group_name'))->urlSlug());
            }
    ...
    public function add_bad_tags()
    {
        ...
        $tag_array = array_unique(array_map(
            'trim',
    ...
    public function tag_field_sync()
    {
        ...
        if ($this->flux->ver_gte(4)) {
            ...
            foreach ($channels as $channel) {
                $tag_fields_in_channel = $channel->getAllCustomFields()->filter('field_type', 'tag')->toArray();

                if (!empty($tag_fields_in_channel) && is_array($tag_fields_in_channel)) {
                    foreach ($tag_fields_in_channel as $index => $field) {
                        $tag_fields[] = array(
    ...
    public function sync_tag_fields()
    {
        ...
        if ($this->flux->ver_gte(4)) {
            // TODO: Update this to use the channel entry
            // $channelEntry = ee('Model')->get('ChannelEntries', ee()->db->escape_str($entry_id))->first();
        ...
        foreach ($tag_fields as $row) {
            ...
            if (! empty($tags)) {
                if ($this->flux->ver_gte(4)) {
                    $entry = ee('Model')->get('ChannelEntry', $entry_id)->filter('site_id', ee()->config->item('site_id'))->first();
                    $entry->{'field_id_' . $field_id} = implode("\n", $tags);
                    $entry->save();
                } else {
                    ee()->db->update(
                        'exp_channel_data',
    ...
    public function code_pack($message = '')
    {
        ...
        $main_section['intro_message'] = array(
            'title'     => lang('demo_templates'),
            'desc'      => lang('demo_description'),
            'fields'    => array()
        );
